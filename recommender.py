...
MODEL_CACHE_PATH = PROJECT_DATA_DIR / f"model_cache_v{CACHE_VERSION}.pkl"
MATCH_THRESHOLD = 3
DECISION_METRIC_WEIGHTS = {
    "accuracy": 1.0 / 6.0,
    "precision": 1.0 / 6.0,
    "recall": 1.0 / 6.0,
    "f1": 1.0 / 6.0,
...
def _resolve_data_file(filename: str) -> Path:
    for candidate in (PROJECT_DATA_DIR / filename, LEGACY_DATA_DIR / filename):
        if candidate.exists():
            return candidate
    return PROJECT_DATA_DIR / filename


DEFAULT_HISTORY_PATH = _resolve_data_file("history.csv")
DEFAULT_INVESTORS_PATH = _resolve_data_file("investors.csv")
DEFAULT_INVENTORS_PATH = _resolve_data_file("inventors.csv")
...
class PlatformRecommender:
    ...
    def train(self) -> None:
        ...
        for name, estimator in model_specs.items():
            ...
            pr_pairs = sorted(
                zip(pr_recall.tolist(), pr_precision.tolist()),
                key=lambda pair: pair[0],
            )
            pr_recall_sorted = [round(float(value[0]), 6) for value in pr_pairs]
            pr_precision_sorted = [round(float(value[1]), 6) for value in pr_pairs]
            ...
            metrics["class_score"] = float(
                0.25 * metrics["accuracy"]
                + 0.25 * metrics["precision"]
                + 0.25 * metrics["recall"]
                + 0.25 * metrics["f1"]
            )
            metrics["curve_score"] = float(
                0.5 * metrics["roc_auc"] + 0.5 * metrics["pr_auc"]
            )
            metrics["decision_score"] = float(
                sum(metrics[key] * weight for key, weight in DECISION_METRIC_WEIGHTS.items())
            )
            metrics["presentation_score"] = metrics["decision_score"]
            metrics["overall"] = metrics["decision_score"]
            ...
            curve_payloads.append(
                {
                    "name": name,
                    "roc": {
                        "fpr": [round(float(value), 6) for value in roc_fpr],
                        "tpr": [round(float(value), 6) for value in roc_tpr],
                    },
                    "pr": {
                        "recall": pr_recall_sorted,
                        "precision": pr_precision_sorted,
                    },
                    "roc_auc": round(metrics["roc_auc"] * 100, 2),
                    "pr_auc": round(metrics["pr_auc"] * 100, 2),
                }
            )

        self.best_model_name = max(
            self.models.values(),
            key=lambda item: item.metrics["decision_score"],
        ).name
        comparison = self.get_model_comparison()
        curve_lookup = {row["name"]: row for row in curve_payloads}
        self.graph_payload = {
            ...
            "pr_auc": [round(row["pr_auc"] * 100, 2) for row in comparison],
            "decision_score": [round(row["decision_score"] * 100, 2) for row in comparison],
            "confusion": [
                {"name": row["name"], **row["confusion"]} for row in comparison
            ],
            "curves": [curve_lookup[row["name"]] for row in comparison],
            "positive_rate": round(float(y_test.mean()) * 100, 2),
        }
    ...
    def _load_model_cache(self) -> bool:
        ...
        try:
            ...
            self.graph_payload = payload.get("graph_payload", {})
            return True
        except Exception:
            return False
    ...
    def get_model_comparison(self) -> List[Dict[str, float]]:
        ordered = sorted(
            ...
            key=lambda row: row["decision_score"],
            reverse=True,
        )
        return ordered
